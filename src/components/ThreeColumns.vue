  ...
  >
    ...
    >
      <draggable
        ...
        :options="{group:'people', ghostClass: 'ghost'}"
        @end="handleMoveEnd"
        @add="handleDivWidgetAdd($event, element.columns[1])"
      >
    ...
    <div
      :style="{
        width: element.options.widthright + 'px',  
      height: element.options.height + 'px'
        } "
      :class="{active: selectWidget.key == element.key}"
      class="right-fixed"
    >
      <draggable
        class="widget-form-list"
        style="padding-bottom: 50px;"
        v-model="element.columns[2].list"
        filter="widget-grid-container"
        :options="{group:'people', ghostClass: 'ghost'}"
        @end="handleMoveEnd"
        @add="handleDivWidgetAdd($event, element.columns[2])"
      >
        <template v-if="element.columns[2].list.length" v-for="(el, i) in element.columns[2].list">
          <template v-if="el.islayout">
            <component
              v-if="el.key"
              :is="el.type"
...
export default {
  ...
  computed: {
    ...
    marginRight() {
      return (
        parseInt(this.element.options.widthright) +
        parseInt(this.element.options.gutter) +
        "px"
      );
    }
